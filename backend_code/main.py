...
def create_app():
    ...
    def sync_tenant_orders(tenant_key):
        ...
        # 优化点：只同步最近2小时内的 PENDING 订单，避免因过多过期无效订单导致请求卡死
        try:
            import sqlite3
            ...
            with sqlite3.connect(quota_service.db_path) as conn:
                cursor = conn.cursor()
                cursor.execute(
                    "SELECT order_id FROM payment_orders WHERE tenant_key = ? AND status = 'PENDING' AND created_at > ?", 
                    (tenant_key, cutoff_time)
                )
